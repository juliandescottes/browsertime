...
class Measure {
  ...
  async stop(testedStartUrl) {
    ...
    if (this.options.safari && this.options.safari.ios) {
      // See https://github.com/danielpaulus/quicktime_video_hack/issues/96
    ...
    if (this.options.safari && this.options.safari.ios) {
      // See https://github.com/danielpaulus/quicktime_video_hack/issues/96
      await this.collect(url);
      return this._stopVideo(url);
    } else {
